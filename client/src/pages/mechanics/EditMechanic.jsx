...
export function EditMechanic() {
  ...
  useEffect(() => {
    fetch("http://localhost:3001/api/mechanics/" + mechanicId, {
      method: "GET",
      headers: {
      ...
      .then(data => {
        if (data.status === "ok") {
          ...
          setPrice(mechanic.price)
          setColor(mechanic.color)
          setMechanicType(mechanic.mechanicType)
          setYear(mechanic.year)
          setSteeringWheel(mechanic.steeringWheel)
